...
endsw
C
C --- Create HYCOM SST data file from a monthly atmos. climatology.
C
switch ($OS)
case 'SunOS':
    if (-e /scr) then
...
touch      fort.71
/bin/rm -f fort.71
ln -s    ~metzger/force/${N}/monthly/${W}_soiltm.D fort.71  &
#${pget} ~metzger/force/${N}/monthly/${W}_soiltm.D fort.71  &
C
...
./kp <<E-o-D
 &AFTITL
  ...
  CTITLE = "${W} soiltm",
  CNAME  = 'surtmp',  !changed to seatmp outside kp
 &END
 &AFTIME
  PARMAX =   99.0,  !disable maximum
  PARMIN =   -1.8,  !minimum is under ice
  PAROFF =    0.0,  !already in degC
  PAROFF = -273.15, !degK to degC
...
endsw
C
C --- Output, note that this is SST from land/ice/sea temperature.
C
sed -e "s/surtmp/seatmp/g" fort.10 >! ./seatmp.b
/bin/mv                    fort.10A   ./seatmp.a
C
${pput} seatmp.b ${D}/seatmp.b
${pput} seatmp.a ${D}/seatmp.a
C
C --- Delete all files.
C
/bin/rm -f fort.* ./kp
