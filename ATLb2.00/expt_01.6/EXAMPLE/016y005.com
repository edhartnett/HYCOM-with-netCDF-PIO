...
C --- 01.5 - repeat 1.3 with src_2.1.03.
C --- 01.6 - repeat 1.5 with src_2.1.34.
C
...
setenv K 22
setenv E 016
setenv P hycom/${R}/expt_01.6/data
setenv D ~/$P
C
switch ($OS)
case 'SunOS':
...
if ($OF != "") then
  ...
  if (-z forcing.offlux.a) then
     ${pget} ${D}/../../force/${FN}/offlux${OF}.a forcing.offlux.a &
  endif
  if (-z forcing.offlux.b) then
     ${pget} ${D}/../../force/${FN}/offlux${OF}.b forcing.offlux.b &
  endif 
endif 
C
...
C
C ---   next segments surtmp already exist.
C
    else
      cd ./ssta
...
C
C ---   next segments seatmp already exist.
C
    else
      cd ./ssto
...
C
  ...
  if ( `tail -1 summary_out | grep -c "^normal stop"` == 1 ) then
    ...
    /bin/rm -f ./flxt/*_${Y01}${A}*
    /bin/rm -f ./pcip/*_${Y01}${A}*
    /bin/rm -f ./ssta/*_${Y01}${A}*
    /bin/rm -f ./ssto/*_${Y01}${A}*
  endif
C
