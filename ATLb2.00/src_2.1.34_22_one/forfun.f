...
c
c --- units of tau_x  are N/sq_m (positive eastwards  w.r.t. the grid)
c --- units of tau_y  are N/sq_m (positive northwards w.r.t. the grid)
c --- units of wndspd are m/s
c --- units of ustar  are sqrt(N.m/kg)
c --- units of airtmp are degC
c --- units of vapmix are kg/kg
c --- units of precip are m/s    (positive into ocean)
c --- units of radflx are w/sq_m (positive into ocean)
c --- units of swflx  are w/sq_m (positive into ocean)
c --- units of offlux are w/sq_m (positive into ocean)
c
...
c
      ...
      if     (mnproc.eq.1) then  ! .b file from 1st tile only
      open (unit=900,file=flnmfor(1:lgth)//'forcing.ustar.b',
     &   status='old', action='read')
      read (900,'(a79)') preambl
      endif !1st tile
      call preambl_print(preambl)
...
c
     ...
     &        icmflg.eq.2 .or. ticegr.eq.0.0     ) then
      ...
      if     (mnproc.eq.1) then  ! .b file from 1st tile only
      open (unit=909,file=flnmfor(1:lgth)//'forcing.surtmp.b',
     &   status='old', action='read')
      read (909,'(a79)') preambl
      endif !1st tile
      call preambl_print(preambl)
...
c
      ...
      if     (mnproc.eq.1) then  ! .b file from 1st tile only
      open (unit=910,file=flnmfor(1:lgth)//'forcing.seatmp.b',
     &   status='old', action='read')
...
c
      ...
      if     (mnproc.eq.1) then  ! .b file from 1st tile only
      open (unit=916,file=flnmfor(1:lgth)//'forcing.offlux.b',
     &   status='old', action='read')
      read (916,'(a79)') preambl
      endif !1st tile
      call preambl_print(preambl)
...
c
      if (thermo) then
        ...
        if     (mnproc.eq.1) then  ! .b file from 1st tile only
        open (unit=919,file=flnmfor(1:lgth)//'forcing.kpar.b',
     &     status='old', action='read')
        read (919,'(a79)') preambl
        endif !1st tile
        call preambl_print(preambl)
...
c
      if     (.not.priver) then
        ...
        if     (mnproc.eq.1) then  ! .b file from 1st tile only
        open (unit=918,file=flnmfor(1:lgth)//'forcing.rivers.b',
     &     status='old', action='read')
        read (918,'(a79)') preambl
        endif !1st tile
        call preambl_print(preambl)
...
c
      if     (relax) then  ! boundary thermal relaxation
        ...
        if     (mnproc.eq.1) then  ! .b file from 1st tile only
        open (unit=915,file=flnmforw(1:lgth)//'relax.rmu.b',
     &        status='old', action='read')
        read (915,'(a79)') preambl
        endif !1st tile
        call preambl_print(preambl)
...
c
      if     (trcrlx) then  ! boundary tracer relaxation
        ...
        if     (mnproc.eq.1) then  ! .b file from 1st tile only
        open (unit=915,file=flnmforw(1:lgth)//'relax.rmutr.b',
     &        status='old', action='read')
        endif !1st tile
        call zagetc(cline,ios, 915)  !1st line of the header on all tiles
        if     (mnproc.eq.1) then  ! .b file from 1st tile only
        rewind 915
        read  (915,'(a79)') preambl
        endif !1st tile
...
c
      ...
      if     (mnproc.eq.1) then  ! .b file from 1st tile only
      open (unit=924,file=flnmfor(1:lgth)//'iso.top.b',
     &   status='old', action='read')
      endif !1st tile
...
c
      ...
      if     (mnproc.eq.1) then  ! .b file from 1st tile only
      open (unit=922,file=flnmfor(1:lgth)//'iso.sigma.b',
     &   status='old', action='read')
      endif !1st tile
      do k= 1,kk
...
c
        ...
        if     (mnproc.eq.1) then  ! .b file from 1st tile only
        open (unit=923,file=flnmfor(1:lgth)//'veldf2.b',
     &     status='old', action='read')
        endif !1st tile
...
c
        ...
        if     (mnproc.eq.1) then  ! .b file from 1st tile only
        open (unit=923,file=flnmfor(1:lgth)//'veldf4.b',
     &     status='old', action='read')
        endif !1st tile
...
c
        ...
        if     (mnproc.eq.1) then  ! .b file from 1st tile only
        open (unit=923,file=flnmfor(1:lgth)//'thkdf4.b',
     &     status='old', action='read')
        endif !1st tile
        ...
        if     (sstflg.ne.3) then  !use atmos. sst as "truth"
          do j= 1-nbdy,jj+nbdy
            do i= 1-nbdy,ii+nbdy
              ...
              seatmp(i,j,2) = max( sstmin, min(surtmp(i,j,1), sstmax ) )
            enddo
          enddo
        endif
      else
...
!$OMP&           SCHEDULE(STATIC,jblk)
      ...
      if     (thermo) then
        if     (ustflg.eq.3) then
          call rdmonth(ustara(1-nbdy,1-nbdy,lslot),900)
        endif
        if (wndflg.ne.3) then
...
c
      if     (lwflag.eq.2 .or. sstflg.eq.2   .or.
     &        icmflg.eq.2 .or. ticegr.eq.0.0     ) then
        call rdmonth(surtmp(1-nbdy,1-nbdy,lslot),909)
        if     (sstflg.ne.3) then
          do j= 1-nbdy,jj+nbdy
            do i= 1-nbdy,ii+nbdy
...
c
      if     (sstflg.eq.3) then
        call rdmonth(seatmp(1-nbdy,1-nbdy,lslot),910)
      endif
c
      mreca = mnth
c
...
c --- read desired month
c
      call rdmonth(akpar(1-nbdy,1-nbdy,lslot),919)
c
      mreck = mnth
c
      if     (mnproc.eq.1) then
...
c --- read desired month
c
      call rdmonth(rivers(1-nbdy,1-nbdy,lslot),918)
c
      mrecr = mnth
c
      if     (mnproc.eq.1) then
          ...
          do j= 1,jj
            do i= 1,ii
              if     (depths(i,j).gt.shallow) then
                p23min(1) = min( p23min(1), pwall(i,j,2,lslot) )
                p23min(2) = min( p23min(2), pwall(i,j,3,lslot) )
              endif
            enddo
          enddo
          call xcminr(p23min)
...
c
      ...
      integer   i,idmtst,ios,j,jdmtst,k
...
*
      ...
      if     (mnproc.eq.1) then  ! .b file from 1st tile only
        open (unit=921,file=flnm//'.b',form='formatted',
     &        status='old',action='read')
c
        read(921,'(a)') cline
        read(921,'(a)') cline
...
c
      call rd_archive(util1, cfield, 921)  ! montg1
     ...
     &         'error in rdbaro_in - expected ','montg1  '
      ...
      call rd_archive(util2, cfield, 921)  ! srfhgt=montg1+thref*pbnest
     ...
     &         'error in rdbaro_in - expected ','srfhgt  '
      ...
      do i= 1,9
        ...
        call zaiosk(921)
      enddo
      call rd_archive(ubnest(1-nbdy,1-nbdy,lslot), cfield, 921)
     ...
     &         'error in rdbaro_in - expected ','u_btrop '
      ...
      call rd_archive(vbnest(1-nbdy,1-nbdy,lslot), cfield, 921)
      ...
      do j=1,jj
        do i=1,ii
          if     (ip(i,j).eq.1) then
            ...
            ubpnst(i,j,lslot) = 0.5*(ubnest(i,  j,lslot)+
     &                               ubnest(i+1,j,lslot) )
            vbpnst(i,j,lslot) = 0.5*(vbnest(i,j,  lslot)+
     &                               vbnest(i,j+1,lslot) )
          else
...
c
        ...
        if     (mnproc.eq.1) then  ! .b file from 1st tile only
        open (unit=915,file='nest/rmu.b',status='old',action='read')
        endif !1st tile
        call zagetc(cline,ios, 915)  !1st line of the header on all tiles
        if     (mnproc.eq.1) then  ! .b file from 1st tile only
        rewind 915
...
*
      ...
      if     (mnproc.eq.1) then  ! .b file from 1st tile only
        open (unit=920,file=flnm//'.b',form='formatted',
     &        status='old',action='read')
c
        read(920,'(a)') cline
        read(920,'(a)') cline
...
c
      do i= 1,11
        ...
        call zaiosk(920)
      enddo
      call zagetc(cline,ios, 920)  !kemix or covice or u_btrop
      meanar = cline(1:8).eq.'kemix   '
      if     (meanar) then
        call zaiosk(920)  !skip kemix
        call rd_archive(util1, cfield, 920) !covice or u_btrop
        if     (cfield.eq.'covice  ') then
          if     (mnproc.eq.1) then  ! .b file from 1st tile only
            read (920,*)
          endif
          ...
          call rd_archive(util1, cfield, 920) !u_btrop
        endif
        call rd_archive(util2, cfield, 920) !v_btrop
        if     (mnproc.eq.1) then  ! .b file from 1st tile only
          read (920,*)
        endif
...
c
     ...
     &           'error in rdnest_in - expected ','v-vel.  '
        ...
        if     (k.ne.kk) then
          call rd_archive(pnest(1-nbdy,1-nbdy,k+1,lslot), cfield, 920)
     ...
     &             'error in rdnest_in - expected ','thknss  '
        ...
        call rd_archive(tnest(1-nbdy,1-nbdy,k,lslot), cfield, 920)
     ...
     &           'error in rdnest_in - expected ','temp    '
        ...
        call rd_archive(snest(1-nbdy,1-nbdy,k,lslot), cfield, 920)
...
c
c
      subroutine rd_archive(field, cfield, iunit)
      use mod_xc  ! HYCOM communication interface
      use mod_za  ! HYCOM I/O interface
      implicit none
