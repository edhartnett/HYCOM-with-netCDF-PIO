...
c ---       reposition file for tracer input
            ...
            if     (lmyin) then
              kskip = 12*kdm+14+2*kapnum + 7*kdm+14
            else
...
c
      ...
      subroutine restart_out(nstepx, dtimex, last)
      use mod_xc  ! HYCOM communication interface
      use mod_za  ! HYCOM I/O interface
      implicit none
c
...
c
      icount = icount + 1
c
      if     (last .or. mod(icount,2).eq.0) then
        iunit = 12  ! standard restart file
      else
        iunit = 22  ! backup   restart file
      endif
c
      call zaiopi(lopen, iunit)
      if (.not.lopen) then
        if     (iunit.eq.12) then
          call zaiopf(flnmrso(1:len_trim(flnmrso))//'.a','new', iunit)
        else
          call zaiopf(flnmrso(1:len_trim(flnmrso))//'1.a','new', iunit)
        endif
        if     (mnproc.eq.1) then
          if     (iunit.eq.12) then
            open (unit=iunit,file=flnmrso(1:len_trim(flnmrso))//'.b',
     &            status='new',action='write',form='formatted')
            write(lp,'(a)') ' creating a new standard restart file'
          else
            open (unit=iunit,file=flnmrso(1:len_trim(flnmrso))//'1.b',
     &            status='new',action='write',form='formatted')
...
c
      if     (mnproc.eq.1) then
      write(iunit,'(a,3i6)') 'RESTART: iexpt,iversn,yrflag = ',
     &                              iexpt,iversn,yrflag
      write(cline,*)                nstepx,dtimex
      write(iunit,'(a,a)')   'RESTART: nstep,dtime = ',
     &                              cline(1:len_trim(cline))
      call flush(iunit)
      endif !1st tile
c
      call zaiowr3(u,      2*kdm, iu,.false., xmin,xmax, iunit,.true.)
      ...
      call zaiowr3(v,      2*kdm, iv,.false., xmin,xmax, iunit,.true.)
      ...
      call zaiowr3(dp,     2*kdm, ip,.false., xmin,xmax, iunit,.true.)
      ...
      call zaiowr3(temp,   2*kdm, ip,.false., xmin,xmax, iunit,.true.)
      ...
      call zaiowr3(saln,   2*kdm, ip,.false., xmin,xmax, iunit,.true.)
      ...
      call zaiowr3(th3d,   2*kdm, ip,.false., xmin,xmax, iunit,.true.)
      call xctilr( th3d, 1,2*kdm, nbdy,nbdy, halo_ps)
      if     (mnproc.eq.1) then
      do l= 0,1
        do k= 1,kdm
          write(iunit,4100) 'th3d    ',k,l+1,xmin(k+l*kdm),xmax(k+l*kdm)
        enddo
      enddo
      call flush(iunit)
      endif !1st tile
c
      if (mxlmy) then
        call zaiowr3(q2, 2*kdm+4, ip,.false., xmin,xmax, iunit,.true.)
        if     (mnproc.eq.1) then
        do l= 0,1
          do k= 1,kdm+2
            write(iunit,4100) 'q2      '
     &                    ,k,l+1,xmin(k+l*(kdm+2)),xmax(k+l*(kdm+2))
        ...
        call zaiowr3(q2l, 2*kdm+4, ip,.false., xmin,xmax, iunit,.true.)
        if     (mnproc.eq.1) then
        do l= 0,1
          do k= 1,kdm+2
     ...
     &                    ,k,l+1,xmin(k+l*(kdm+2)),xmax(k+l*(kdm+2))
        ...
        call zaiowr3(vctymy, kdm+2, ip,.false., xmin,xmax, iunit,.true.)
        if     (mnproc.eq.1) then
        do l= 1,1
          do k= 1,kdm+2
        ...
        call zaiowr3(difqmy, kdm+2, ip,.false., xmin,xmax, iunit,.true.)
        if     (mnproc.eq.1) then
        do l= 1,1
          do k= 1,kdm+2
        ...
        call zaiowr3(diftmy, kdm+2, ip,.false., xmin,xmax, iunit,.true.)
        if     (mnproc.eq.1) then
        do l= 1,1
          do k= 1,kdm+2
...
c
      call zaiowr3(ubavg,      3, iu,.false., xmin,xmax, iunit,.true.)
      ...
      call zaiowr3(vbavg,      3, iv,.false., xmin,xmax, iunit,.true.)
      ...
      call zaiowr3(pbavg,      3, ip,.false., xmin,xmax, iunit,.true.)
      ...
      call zaiowr3(pbot,       1, ip,.false., xmin,xmax, iunit,.true.)
      ...
      call zaiowr3(psikk,  kapnum,ip,.false., xmin,xmax, iunit,.true.)
      ...
      call zaiowr3(thkk,  kapnum, ip,.false., xmin,xmax, iunit,.true.)
      ...
      call zaiowr3(dpmixl,     2, ip,.false., xmin,xmax, iunit,.true.)
      ...
      if (icegln) then
        call zaiowr3(temice,     1, ip,.false., xmin,xmax, iunit,.true.)
        ...
        call zaiowr3(covice,     1, ip,.false., xmin,xmax, iunit,.true.)
        ...
        call zaiowr3(thkice,     1, ip,.false., xmin,xmax, iunit,.true.)
