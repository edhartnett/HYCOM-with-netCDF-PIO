...
cdiag end do
cdiag write (lp,'(i9,2i5,3x,a,1p,3e12.4/22x,a,3e12.4)') nstep,itest,
cdiag.  jtest,'total saln,srf.flux,tndcy:',tosal/g,salflx(itest,
...
c
     ...
     .     thknss,ustar3,
     .     dtemp2,q,swfold,thet,
     .     swfrac,sflux1,tmin,tmax,smin,smax,trmin,trmax,
     .     thkold,thknew,t1,t2,s1,s2,tr2,dp1,dp2,dtrmax
c
...
c --- if mixed layer base is very close to interface, move it there
      if (abs(dpmixl(i,j,n)-dp(i,j,1,n)).lt..001*dp(i,j,1,n)) then
        dpmixl(i,j,n)=dp(i,j,1,n)+onecm
      endif
c
...
c
      ...
      if(klist(i,j).ge.2) then
        ...
        th3d(i,j,1,n)=sig(temp(i,j,1,n),saln(i,j,1,n))-thbase
        if(trcout) then
          trmin=min(1.0,tracer(i,j,k),tracer(i,j,k0))
          trmax=max(1.0,tracer(i,j,k),tracer(i,j,k0))
          tr2=tracer(i,j,k)+q*(1.0-tracer(i,j,k))
          tracer(i,j,k)=min(trmax,max(trmin,tr2))
        end if
      end if
13    continue
c
...
ccc   turgen(i,j)=delt1*(2.*em*g*ustar3/thref+thkold*thermg)/thref**2
c
...
c
      ...
      ape=cp3*ustar3+cp1*dpth*buoyfl(i,j)
c
      if(ape.lt.0.) then                                       ! detrainment
      turgen(i,j)=(g*delt1/thref**3)*ape
      depnew(i,j)=min(thkold,g*cp3/(thref*cp1*max(epsil,obuinv)))
c
      else                                                     ! entrainment
      cc4=2.*em4/(em1*em1) * alf1*alf1
      spe=(em2+em3)*ustar3+0.5*dpth*buoyfl(i,j)
      turgen(i,j)=(g*delt1/thref**3)*(sqrt((.5*ape-cp1*spe)**2
...
cdiag     write (lp,'(i9,2i5,a,1p,2e13.5)') nstep,i,j,
cdiag.    '  m-o length (m), turgen:',depnew(i,j)/onem,turgen(i,j)
cdiag   else
cdiag     write (lp,'(i9,2i5,a,1p,2e13.5)') nstep,i,j,
cdiag.    '  new depth (m), turgen:',depnew(i,j)/onem,turgen(i,j)
cdiag   endif
cdiag endif
c
...
c --- detrainment rate limited to dtrate m/day
     ...
     .            max(thkmin*onem,dp(i,j,1,n)+onemm,
...
c ---   penetrating solar radiation (all redfac in mixed layer)
        if (thknew.lt.p(i,j,kk+1)-onecm) then
          swfrac=
     .       (1.-redfac(jerlov(i,j)))*exp(-thknew/betabl(jerlov(i,j)))
        else
c ---     mixed layer reaches the bottom
     ...
     .                   thknew
cdiag if (i.eq.itest.and.j.eq.jtest) write (lp,104) nstep,i,j,k,
cdiag.  0.,1.-swfrac,dtemp(i),dsaln(i)
 104  format(i9,3i4,'absorbup,dn,dtemp,dsaln ',2f6.3,2f10.6)
c
      else
c
...
c
      ...
      if(trcout) then
        tracer(i,j,k)=tracer(i,j,k)+q*(1.0-tracer(i,j,k))
      endif
 9    continue
c
...
c ---     penetrating solar radiation (all redfac in mixed layer)
          if     (p(i,j,k+1).lt.p(i,j,kk+1)-onecm) then
            swfold=
     .         (1.-redfac(jerlov(i,j)))*
     .             exp(-max(thknss,p(i,j,k  ))/betabl(jerlov(i,j)))
            swfrac=
     .         (1.-redfac(jerlov(i,j)))*
     .             exp(           -p(i,j,k+1) /betabl(jerlov(i,j)))
...
cdiag.   (lp,'(i9,2i4,i3,a,f9.3,2f8.3)') nstep,i,j,k,
cdiag.   ' dpu, old/new u ',dpu(i,j,k,n)/onem,uold,u(i,j,k,n)
 32   continue
c
...
cdiag if (i.eq.itest .and. j.eq.jtest) write
cdiag.   (lp,'(i9,2i4,i3,a,f9.3,2f8.3)') nstep,i,j,k,
cdiag.   ' dpv, old/new v ',dpv(i,j,k,n)/onem,vold,v(i,j,k,n)
 52   continue
c
 31   continue
c
...
c
      real    dpth,ekminv,obuinv,ex,alf1,alf2,cp1,cp3,ape,cc4,spe,
     .        ustar3,thkold,thknew,value,q,tdp,sdp,tem,sal,rho,
...
c
      ...
      if (pres(k+1).gt.thkold) then
        value=(2.*turgen(i,j)+dens(k)*pres(k)**2-sum2)/
...
c
        sum1=sum1+dens(k)*(pres(k+1)   -max(pres(k),thkold)   )
        sum2=sum2+dens(k)*(pres(k+1)**2-max(pres(k),thkold)**2)
      end if
 85   continue
c
cdiag if (i.eq.itest .and. j.eq.jtest .and. turgen(i,j).lt.0.)
cdiag.  write (lp,'(i9,2i5,a,f8.2,1p,e13.3)') nstep,itest,jtest,
cdiag.  '  monin-obukhov length (m),turgen:',thknew/onem,turgen(i,j)
c
...
cdiag   write (lp,'(2i4,a,1p,2e16.8,e9.1)') i,j,
...
cdiag.  nstep,i,j,'total saln,srf.flux,tndcy:',tosal/g,
...
cdiag.      dpu(i,j,k,n)/onem,zup/onem,zlo/onem,s1,s2,s3,
...
cdiag.      dpv(i,j,k,n)/onem,zup/onem,zlo/onem,s1,s2,s3,
