...
c --- kdm   =       grid dimension in k direction
      ...
      parameter (itdm= 198,jtdm= 310,kdm=15)  ! JESd0.08
c
c --- iqr   = maximum number of tiles in i direction
c --- jqr   = maximum number of tiles in j direction
      integer    iqr,jqr
      parameter (iqr=10,jqr=12)  ! multiple tiles (TYPE=ompi or mpi or shmem)
c
...
*     parameter (idm=itdm,jdm=jtdm)  ! always works if enough memory
      parameter (idm=80,jdm=76)      ! NMPI=16,24,32,48,64
c
c --- mxthrd= maximum number of OpenMP threads
      integer    mxthrd
      parameter (mxthrd=12)  ! NOMP=0,1,2,3,4,6,8,12
c
