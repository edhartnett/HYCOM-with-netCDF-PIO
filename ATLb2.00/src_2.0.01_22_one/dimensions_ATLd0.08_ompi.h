...
c --- kdm   =       grid dimension in k direction
      ...
      parameter (itdm=1678,jtdm=1609,kdm=26)  ! ATLd0.08
c
c --- iqr   = maximum number of tiles in i direction
c --- jqr   = maximum number of tiles in j direction
      integer    iqr,jqr
      parameter (iqr=20,jqr=16)  ! multiple tiles (TYPE=ompi or mpi or shmem)
c
...
*     parameter (idm=itdm,jdm=jtdm)  ! always works if enough memory
      parameter (idm=705, jdm=136)   ! 4x16 to 16x16 equal-ocean tiles
c
