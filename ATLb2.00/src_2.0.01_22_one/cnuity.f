...
c
      ...
      real    q,dpmin,clip,flxhi,flxlo,dtinv,dpup,dpdn
      real    dpkmin(2*kdm)
c
      character text*12
c
...
c ---     compare two model runs.
          do j=1-margin,jj+margin
            do i=1-margin,ii+margin
              mask(i,j)=iu(i,j)
              if (i.gt. 1) mask(i,j)=mask(i,j)+iu(i-1,j)
              if (i.lt.ii) mask(i,j)=mask(i,j)+iu(i+1,j)
            enddo
          enddo
          write (text,'(a9,i3)') 'uflux  k=',k
          call pipe_compare(uflux,mask,text)
          write (text,'(a9,i3)') 'uflux2 k=',k
          call pipe_compare(uflux2,mask,text)
          do j=1-margin,jj+margin
            do i=1-margin,ii+margin
              mask(i,j)=iv(i,j)
              if (j.gt. 1) mask(i,j)=mask(i,j)+iv(i,j-1)
              if (j.lt.jj) mask(i,j)=mask(i,j)+iv(i,j+1)
            enddo
          enddo
          write (text,'(a9,i3)') 'vflux  k=',k
          call pipe_compare(vflux,mask,text)
          write (text,'(a9,i3)') 'vflux2 k=',k
          call pipe_compare(vflux2,mask,text)
        endif
c
cdiag if (mod(k,15).eq.1) then
cdiag   do i=itest-1,itest+1
cdiag   do j=jtest-1,jtest+1
cdiag   write (lp,101) nstep,i,j,k,dpold(i-1,j,k),uflux(i,j),
...
c
 100  format (i9,' i,j,k=',3i4,' neg. dp (m) in loop ',i2,f9.2)
c
      do k= 1,kk
        dpmin=dpkmin(k)
            ...
            do l=1,isp(j)
              do i=max(1,ifp(j,l)),min(ii,ilp(j,l))
                if (dpoldm(i,j,k).eq.dpmin) then
                  write (lp,100) nstep,i+i0,j+j0,k,19,dpmin/onem
                endif
              enddo
            enddo
          enddo
            ...
            do l=1,isp(j)
              do i=max(1,ifp(j,l)),min(ii,ilp(j,l))
                if (dp(i,j,k,n).eq.dpmin) then
                  write (lp,100) nstep,i+i0,j+j0,k,15,dpmin/onem
                endif
              enddo
            enddo
          enddo
        ...
        if (dpmin.lt.-onecm) then
          do j=1,jj
            do l=1,isp(j)
              do i=max(1,ifp(j,l)),min(ii,ilp(j,l))
                if (dp(i,j,k,n).eq.dpmin) then
                  write (lp,100) nstep,i+i0,j+j0,k,14,dpmin/onem
                endif
              enddo
            enddo
          enddo
...
c
        margin = mbdy - 2
c
!$OMP   PARALLEL DO PRIVATE(j,l,i,flxhi,flxlo)
!$OMP&           SCHEDULE(STATIC,jblk)
        do j=1-margin,jj+margin
c
          do l=1,isu(j)
...
c
     ...
     &                       max(flxlo,
     &                           delt1*thkdff*
     &                             (util1(i-1,j)-util1(i,j))*scuy(i,j)))
...
c
     ...
     &                       max(flxlo,
     &                           delt1*thkdff*
     &                             (util2(i,j-1)-util2(i,j))*scvx(i,j)))
...
c
 800  continue  ! end of thickness diffusion
c
...
!$OMP     END PARALLEL DO
c
        endif  ! end: (thkdff.ne.0.)
c
      endif  ! end: (hybrid .and. mxlkta .and. thermo)
c
c --- cumalative fluxes
c
