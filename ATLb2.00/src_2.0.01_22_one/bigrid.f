      subroutine bigrid(depth, util1,util2,util3)
...
c
c --- is it fully global (across the arctic).
      depmax=0.0
...
c
      if     (.not.larctic .and. j0+jj.eq.jtdm) then
...
c --- data are written in strips nchar points wide
      ...
      do ifrst=0,nchar*isec,nchar
        ...
        fmt(8:10)=char3
        if     (mnproc.eq.1) then
        write (lp,'(''ip array, cols'',i5,'' --'',i5)') ifrst+1,ilast
        endif
        do j= jtdm,1,-1
