...
c ---   compare two model runs.
        call pipe_compare(u1,  iu,'ad:11:u1    ')
        call pipe_compare(flx, iu,'ad:11:flx   ')
        call pipe_compare(v1,  iv,'ad:11:v1    ')
        call pipe_compare(fly, iv,'ad:11:fly   ')
        call pipe_compare(fmx, ip,'ad:11:fmx   ')
        call pipe_compare(fmn, ip,'ad:11:fmn   ')
      endif
c
c --- rhs: u, v, fld+
c --- lhs: flx, fly, fmx, fmn
...
c
      ...
      if     (lpipe .and. lpipe_advem) then
c ---   compare two model runs.
        call pipe_compare(flx, iu,'ad:22:flx   ')
        call pipe_compare(fly, iv,'ad:33:fly   ')
      endif
...
!$OMP END PARALLEL DO
      if     (lpipe .and. lpipe_advem) then
c ---   compare two model runs.
        call pipe_compare(fld, ip,'ad:610:fld  ')
      endif
c
...
!$OMP&         SCHEDULE(STATIC,jblk)
     ...
     &         /(fco(i,j)+fco(i,j-1)+fc(i,j)+fc(i,j-1)+onemu)
          enddo
        enddo
      enddo
!$OMP END PARALLEL DO
      if     (lpipe .and. lpipe_advem) then
c ---   compare two model runs.
        call pipe_compare(flx, iu,'ad: 8:flx   ')
        call pipe_compare(fly, iv,'ad: 8:fly   ')
      endif
c
...
!$OMP&         SCHEDULE(STATIC,jblk)
     ...
     &       ( (onemu-min(0.,flx(i+1,j))+max(0.,flx(i,j))
     &               -min(0.,fly(i,j+1))+max(0.,fly(i,j)) )*dt)
            fln(i,j)=(fld(i,j)-fmn(i,j))*fc(i,j)*scal(i,j)/
     &       ( (onemu+max(0.,flx(i+1,j))-min(0.,flx(i,j))
     &               +max(0.,fly(i,j+1))-min(0.,fly(i,j)) )*dt)
          enddo
        enddo
      enddo
!$OMP END PARALLEL DO
      if     (lpipe .and. lpipe_advem) then
c ---   compare two model runs.
        call pipe_compare(flp, ip,'ad:16:flp   ')
        call pipe_compare(fln, ip,'ad:16:fln   ')
      endif
c
...
!$OMP END PARALLEL DO
      if     (lpipe .and. lpipe_advem) then
c ---   compare two model runs.
        call pipe_compare(flx, iu,'ad:18:flx   ')
        call pipe_compare(fly, iv,'ad:18:fly   ')
      endif
c
cdiag i=itest
cdiag j=jtest
...
!$OMP END PARALLEL DO
      if     (lpipe .and. lpipe_advem) then
c ---   compare two model runs.
        call pipe_compare(flxdiv, ip,'ad:620:flxdv')
        call pipe_compare(fld,    ip,'ad:1620:fld ')
      endif
c
  100 continue
c
...
!$OMP   END PARALLEL DO
        if     (lpipe .and. lpipe_advem) then
c ---     compare two model runs.
          call pipe_compare(fld, ip,'ad:13:fld   ')
        endif
      endif
ccc
ccc --- optional code for checking conservation properties
...
c
     ...
     &    ,offset,factor,pold,pmid,pnew,wts2dp,baclin5
...
c ---        zero for -saln-, 256. for -temp-, 32 for -th3d-
      integer    pdtemp,pdth3d
      parameter (pdtemp=256.0, pdth3d=32.0)
c
      real harmon,a,b
      include 'stmt_fns.h'
c
...
c
      ...
      if (trcout .and. mod(nstep,5).eq.0) then
        call xctilr(tracer( 1-nbdy,1-nbdy,  1),1,kk, 6,6, halo_ps)
      endif
c
      do 81 k=1,kk
c
...
c ---   compare two model runs.
        ...
        call pipe_compare(sold,ip,text)
        write(text,'(a10,i2)') '49:told,k=',k
        call pipe_compare(told,ip,text)
        write(text,'(a10,i2)') '49:utl1,k=',k
        call pipe_compare(util1,ip,text)
        write(text,'(a10,i2)') '49:utl2,k=',k
        call pipe_compare(util2,ip,text)
...
c ---       time smoothing of thermodynamic variables
            pmid=max(0.,dp(i,j,k,m))
...
c ---   compare two model runs.
        ...
        call pipe_compare(temp(1-nbdy,1-nbdy,k,m),ip,text)
        write (text,'(a9,i3)') 'temp.n k=',k
        call pipe_compare(temp(1-nbdy,1-nbdy,k,n),ip,text)
        write (text,'(a9,i3)') 'sold   k=',k
        call pipe_compare(sold,ip,text)
        write (text,'(a9,i3)') 'saln.m k=',k
        call pipe_compare(saln(1-nbdy,1-nbdy,k,m),ip,text)
        write (text,'(a9,i3)') 'saln.n k=',k
        call pipe_compare(saln(1-nbdy,1-nbdy,k,n),ip,text)
        write (text,'(a9,i3)') 'told   k=',k
        call pipe_compare(told,ip,text)
        write (text,'(a9,i3)') 'th3d.m k=',k
        call pipe_compare(th3d(1-nbdy,1-nbdy,k,m),ip,text)
        write (text,'(a9,i3)') 'th3d.n k=',k
        call pipe_compare(th3d(1-nbdy,1-nbdy,k,n),ip,text)
      endif
c
...
cdiag.dp(itest,jtest,k,n)/onem
c
c --- advection of 'ventilation' tracer (every 5 time steps only)
c
      if (k.gt.1 .and. trcout .and. mod(nstep,5).eq.0) then
        baclin5=5.0*baclin
c
c ---   rhs: uflux+, vflux+, dpold, dp.n, tracer
c ---   lhs: util1, util2, tracer
c
        margin = mbdy - 1
c
!$OMP   PARALLEL DO PRIVATE(j,l,i,flxdiv,offset)
!$OMP&         SCHEDULE(STATIC,jblk)
        do j=1-margin,jj+margin
          do l=1,isp(j)
            do i=max(1-margin,ifp(j,l)),min(ii+margin,ilp(j,l))
              flxdiv=(uflx(i+1,j,k)-uflx(i,j,k)
     &               +vflx(i,j+1,k)-vflx(i,j,k))*baclin5*scp2i(i,j)
              util2(i,j)=.5*(dpold(i,j,k)+dp(i,j,k,n)-flxdiv)
              util1(i,j)=.5*(dpold(i,j,k)+dp(i,j,k,n)+flxdiv)
              offset=min(0.,util1(i,j),util2(i,j))
              util2(i,j)=util2(i,j)-offset
              util1(i,j)=util1(i,j)-offset
              tracer(i,j,k)=max(0.,tracer(i,j,k))
            enddo
          enddo
        enddo
!$OMP   END PARALLEL DO
        call advem(2,tracer(1-nbdy,1-nbdy,k),
     &               uflx(1-nbdy,1-nbdy,k),vflx(1-nbdy,1-nbdy,k),
     &               scp2,scp2i,baclin5,util1,util2,util3,util4)
      endif
c
 81   continue  ! k=1,kk
c
...
!$OMP&         SCHEDULE(STATIC,jblk)
      do j=1-margin,jj+margin
        do l=1,isu(j)
          do i=max(1-margin,ifu(j,l)),min(ii+margin,ilu(j,l))
            factor=scuy(i,j)*harmon(max(dp(i-1,j,k,n),onemm)
     &                             ,max(dp(i  ,j,k,n),onemm))
        ...
        do l=1,isv(j)
          do i=max(1-margin,ifv(j,l)),min(ii+margin,ilv(j,l))
            factor=scvx(i,j)*harmon(max(dp(i,j-1,k,n),onemm)
     &                             ,max(dp(i,j  ,k,n),onemm))
            if     (latemp) then
...
c ---     compare two model runs.
          ...
          call pipe_compare(uflux, iu,text)
          write (text,'(a9,i3)') 'uflux2 k=',k
          call pipe_compare(uflux2,iu,text)
          write (text,'(a9,i3)') 'vflux  k=',k
          call pipe_compare(vflux, iv,text)
          write (text,'(a9,i3)') 'vflux2 k=',k
          call pipe_compare(vflux2,iv,text)
        endif
c
...
!$OMP&         SCHEDULE(STATIC,jblk)
      do j=1-margin,jj+margin
        do l=1,isp(j)
          do i=max(1-margin,ifp(j,l)),min(ii+margin,ilp(j,l))
            factor=-temdff*delt1/(scp2(i,j)*max(dp(i,j,k,n),onemm))
