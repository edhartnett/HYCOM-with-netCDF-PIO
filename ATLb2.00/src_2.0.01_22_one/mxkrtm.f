...
cdiag.  '  monin-obukhov length (m):',sdot(itest,jtest)/onem
c
...
!$OMP PARALLEL DO PRIVATE(j,k,l,i)
!$OMP&         SCHEDULE(STATIC,jblk)
      do 883 j=1-margin,jj+margin
      do 883 k=1,kk
      do 883 l=1,isp(j)
      do 883 i=max(1-margin,ifp(j,l)),min(ii+margin,ilp(j,l))
      p(i,j,k+1)=p(i,j,k)+dp(i,j,k,n)
 883  continue
c
...
c
      integer i,k,l
c
      real thknss,ustar3,dpth,ekminv,obuinv,
     .     ex,alf1,alf2,cp1,cp3,ape,cc4,spe,pnew
c
...
ccc   thermg=.5*g*((en+1.)*buoyfl(i,j)+(en-1.)*abs(buoyfl(i,j)))/thref
ccc   turgen(i,j)=delt1*(2.*em*g*ustar3/thref+thknss*thermg)/thref**2
c
...
c
      ...
      pnew=(2.*turgen(i,j)+th3d(i,j,k,n)*p(i,j,k)**2-util2(i,j))/
     .           max(epsil,th3d(i,j,k,n)*p(i,j,k)   -util1(i,j))
...
c
      util1(i,j)=util1(i,j)+th3d(i,j,k,n)*dp(i,j,k,n)
      util2(i,j)=util2(i,j)+th3d(i,j,k,n)*(p(i,j,k+1)**2-p(i,j,k)**2)
 85   continue
      return
      end
...
c --- if sdot > 0, remove water from layers about to be entrained.
      ...
      if (pnew.ge.p(i,j,k+1)) tracer(i,j,k)=0.
c
...
cdiag.  sig(tmxl,smxl),thknss/onem
c
...
cdiag.  nstep,i,j,'  entrain',sdot(i,j)/onem,' m of water'
c
...
c
        z=-sdot(i,j)*min(1.,dtemp/max(epsil,tnew-tn))/onem
...
c --- new (t,s) in layer  k  will be t=(a*z+b)/(z+d), s=(e*z+f)/(z+d).
      a=tnew
      e=s_up
      b=(tn*dpn+    dtemp*sdot(i,j))/onem
      f=(sn*dpn+(s_up-s1)*sdot(i,j))/onem
      d=dpn/onem
c
...
cdiag.   thknss/onem,'sdot,z=',sdot(i,j)/onem,z,'t,s,dp(',k,')=',tn,
cdiag.   sn,dpn/onem,'real root(s):',(work(nu),nu=1,num)
cdiag end if
...
c --- inject 'ventilation' tracer into layer k
      if (trcout) then
      tracer(i,j,k)=(tracer(i,j,k)*dpn-sdot(i,j))
     .                           /(dpn-sdot(i,j))
      end if
c
cdiag if (i.eq.itest.and.j.eq.jtest)
...
cdiag.  ' final mixed-layer t,s,dp ',tmxl,smxl,dp(i,j,1,n)/onem
c
 26   continue
      return
      end
