...
!$OMP&           SCHEDULE(STATIC,jblk)
        do j=1-margin,jj+margin
          do k=1,kk
            tempk=tofsig(theta(k)+thbase,saln0)
            do l=1,isp(j)
              do i=max(1-margin,ifp(j,l)),min(ii+margin,ilp(j,l))
                temp(i,j,k,1)=tempk
                saln(i,j,k,1)=saln0
...
c
     ...
     &         'k,pmin,poflat,theta,realat = ',
     &         k,pmin(k)*qonem,
     &             pinit*qonem,.5*(theta(k)+theta(k+1))+thbase,realat
            call flush(lp)
          endif
c
        else  ! k==kk
...
c
     ...
     &    thstar(itest,jtest,k)+thbase,dp(itest,jtest,k,1)*qonem,
