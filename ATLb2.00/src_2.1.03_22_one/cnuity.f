...
c
              thkdfu = delt1*thkdf4*(aspux(i,j)**3)
              uflux(i,j)=min(flxhi,
     &                       max(flxlo,
     &                           thkdfu*
...
c
              thkdfv = delt1*thkdf4*(aspvy(i,j)**3)
              vflux(i,j)=min(flxhi,
     &                       max(flxlo,
     &                           thkdfv*
...
c
            do l=1,isu(j)
              do i=max(1-margin,ifu(j,l)),min(ii+margin,ilu(j,l))
                thkdfu = delt1*thkdf4*(aspux(i,j)**3)
                uflux(i,j)=thkdfu*(util1(i-1,j)-util1(i,j))*scuy(i,j)
              enddo
            enddo
            do l=1,isv(j)
              do i=max(1-margin,ifv(j,l)),min(ii+margin,ilv(j,l))
                thkdfv = delt1*thkdf4*(aspvy(i,j)**3)
                vflux(i,j)=thkdfv*(util2(i,j-1)-util2(i,j))*scvx(i,j)
              enddo
            enddo
          enddo
!$OMP     END PARALLEL DO
