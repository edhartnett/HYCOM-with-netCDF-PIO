...
c
     ...
     &     sflux1,tsum,ssum,trsum,usum,vsum,dpsum,thup
c
...
c
      do k=1,kk
        if (thermo) then
          if (k.eq.1) then
            if (pensol) then
              if (p1d(k+1).lt.tenm) then
                swfrac(k+1)=redfac(jrlv) *exp(-p1d(k+1)*betard(jrlv))+
     &                  (1.-redfac(jrlv))*exp(-p1d(k+1)*betabl(jrlv))
              else
                swfrac(k+1)=(1.-redfac(jrlv))*
     &                      exp(-p1d(k+1)*betabl(jrlv))
              endif
...
cdiag           call flush(lp)
cdiag         endif
 100          format(i9,3i4,'absorbup,dn,dtemp,dsaln ',2f6.3,2f10.6)
            else !.not.pensol
              dtemp=surflx(i,j)*
...
c
cdiag     if (i.eq.itest .and. j.eq.jtest) then
cdiag       write (lp,101) nstep,i,j,k,kmlb,
cdiag&        ' relieve static instability - t,s,th:',
cdiag&        t1d(k),s1d(k),tr1d(k,1)
cdiag       call flush(lp)
cdiag     endif
 101      format (i9,2i4,2i3,a/9x,3f9.4)
        enddo
      endif
c
...
c
          do k3=2,k
            ...
            v1d (k3)=v1d (1)
c
cdiag       if (i.eq.itest .and. j.eq.jtest .and. k3.eq.k1) then
cdiag         write (lp,102) nstep,i,j,k,k1,k2,k3,kmlb,
cdiag&          ' bulk ri mixing - rib,t,s,th:',min(1000.0,rib),
cdiag&          t1d(k3),s1d(k3),th1d(k3)
cdiag         call flush(lp)
cdiag       endif
 102        format (i9,2i4,5i3,a/9x,4f9.4)
c
          enddo
          kmlb=k
        else
...
c
        do k=kmlb+1,kmax
          rig(k)=g*0.5*(dp1d(k-1)+dp1d(k))*thref*
     &           max(1.0e-3,th1d(k)-th1d(k-1))/(onem*
     &           max(1.0e-6,(u1d(k-1)-u1d(k))**2+(v1d(k-1)-v1d(k))**2))
cdiag     if (i.eq.itest .and. j.eq.jtest) then
cdiag       write(6,103) nstep,i,j,k,iter,rig(k)
cdiag       call flush(lp)
cdiag     endif
 103      format('rig(k)',i9,2i4,2i3,1p,e13.5)
        enddo
c
...
cdiag     if (i.eq.itest .and. j.eq.jtest) then
cdiag       write(6,104) nstep,i,j,k,rigf,rig1,v1d(k-1),v1d(k)
cdiag       call flush(lp)
cdiag     endif
 104      format('rig mix',i9,2i4,i3,1p,4e13.5)
c
        end if
c
      enddo
